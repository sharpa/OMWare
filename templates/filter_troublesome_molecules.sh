...
then
  python $root/Operations/BioNano/CheckMOlecules/prefilter.py $minlen $minlables $output_dir/mol_stats.txt > $output_dir/prefiltered_ids.txt
fi
...
then
  ...
  ### sbatch -o "$output_dir/minimized_align.txt" $root/Operations/BioNano/CheckMolecules/minimize_alignment.sh $align_file_list $minlog10pval ### Uncomment me maybe
...
then
  python $root/Operations/BioNano/CheckMolecules/get_mol_align_stats.py $output_dir/minimized_align.txt $output_dir/mol_stats.py $output_dir/prefiltered.txt > $output_dir/align_stats.txt
fi
